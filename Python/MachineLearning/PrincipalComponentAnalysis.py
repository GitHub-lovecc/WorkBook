...
由于新坐标轴的方差逐渐递减，我们会发现大部分的方差都包含在最前面的几个坐标轴中，
因此我们忽视余下的坐标轴（这就是以损失一部分信息为代价）即完成数据的降维。
原始数据有n个维度，我们就能找到重要性依次递减的n条坐标轴。
...
Because the variance of the new coordinate axis decreases gradually, we will find that most of the variance is contained in the first few coordinate axes.
...
The original data has n dimensions, so we can find n coordinate axes with decreasing importance.
